...
async function compressImage(base64) {
  try {
    ...
    const compressed = await sharp(buffer).resize(800, 800, { fit: 'inside' }).jpeg({ quality: 70 }).toBuffer();
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/api/update-profile' && req.method === 'POST') {
    ...
    req.on('end', () => {
      ...
      try {
        ...
        if (users[username]) { if (nickname) users[username].nickname = nickname; if (avatar) users[username].avatar = avatar; if (avatarType) users[username].avatarType = avatarType; saveUsers(); }
        res.writeHead(200); res.end(JSON.stringify({ success: true }));
      } catch(e) { res.writeHead(500); res.end(JSON.stringify({ success: false })); }
    });
    return;
  ...
  if (pathname === '/api/favorites' && req.method === 'GET') {
    ...
    res.writeHead(200);
    res.end(JSON.stringify({ success: true, favorites: messages.filter(m => userFavs.includes(m.msgId)), favIds: userFavs }));
    return;
  }
  ...
  ws.on('message', async (raw) => {
    ...
    switch(data.type) {
      ...
      case 'image': {
        ...
        const msg = { type: 'message', msgId: data.msgId || getMsgId(), imageUrl: 'data:image/jpeg;base64,' + compressed, sender: client.nickname, senderUsername: client.username, avatar: client.avatar, avatarType: client.avatarType, timestamp: Date.now(), clientId: id, recalled: false };
        messages.push(msg);
        broadcastAll(msg);
        saveMessages();
        break;
      ...
      case 'audio': {
        ...
        const msg = { type: 'message', msgId: getMsgId(), audioUrl: 'data:audio/webm;base64,' + data.audio, sender: client.nickname, senderUsername: client.username, avatar: client.avatar, avatarType: client.avatarType, timestamp: Date.now(), clientId: id, recalled: false };
        messages.push(msg);
        broadcastAll(msg);
        saveMessages();
        break;
      ...
      case 'video': {
        ...
        const msg = { type: 'message', msgId: data.msgId || getMsgId(), videoUrl: data.video, sender: client.nickname, senderUsername: client.username, avatar: client.avatar, avatarType: client.avatarType, timestamp: Date.now(), clientId: id, recalled: false };
        messages.push(msg);
        broadcastAll(msg);
        saveMessages();
        break;
